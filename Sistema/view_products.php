...
<head>
  ...
  <title>Página Principal - Página de Compras</title>
</head>
<body>
  ...
  <div class="main">
    ...
    <div class="menu-container">
        <nav>
            <ul>
                <li><a href="javascript:void(0);" onclick="showCategory('sandwich')">Sandwich</a></li>
                <li><a href="javascript:void(0);" onclick="showCategory('segundos')">Segundos</a></li>
                <li><a href="javascript:void(0);" onclick="showCategory('sopas')">Sopas</a></li>
                <li><a href="javascript:void(0);" onclick="showCategory('bebidas')">Bebidas</a></li>
                <li><a href="javascript:void(0);" onclick="showCategory('aperitivos')">Aperitivos</a></li>
            </ul>
        </nav>
    </div>
</section>
...
<section class="products">
    <?php
    $categorias = ['sandwich', 'segundos', 'sopas', 'bebidas', 'aperitivos'];
    foreach ($categorias as $categoria) {
        echo "<div class='category' id='{$categoria}' style='display: none;'>"; // Añadir 'display: none' inicialmente
        echo "<h2>" . ucfirst($categoria) . "</h2>";
        echo "<div class='box-container'>";
        
        $select_products = $conn->prepare("SELECT * FROM productos WHERE categoria = ?");
        $select_products->execute([$categoria]);

        if ($select_products->rowCount() > 0) {
            while ($fetch_products = $select_products->fetch(PDO::FETCH_ASSOC)) {
                ?>
                <form id="generateQrForm_<?php echo $fetch_products['id_producto']; ?>" class="box">
    <img src="data:image/jpeg;base64,<?php echo base64_encode($fetch_products['imagen_producto']); ?>" alt="Producto" class="img">
    <div class="detail">
        ...
        <div class="flex">
            <a href="view_page.php?pid=<?php echo $fetch_products['id_producto']; ?>" class="btn">Ver Detalles</a>
        </div>
    ...
    -->
        ...
        echo "</div>";
        echo "</div>"; // Cerrar div de la categoría
    }
    ?>
</section>
  ...
  <script src="../scriptsS/menu.js"></script>
